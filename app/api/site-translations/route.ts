...
import {
  getOrCreateSiteTranslationResult,
...
export async function GET(request: NextRequest) {
  ...
  if (!namespace || !isKnownSiteTranslationNamespace(namespace)) {
    return NextResponse.json({ error: "Unsupported namespace" }, { status: 400 });
  }

  const result = await getOrCreateSiteTranslationResult(language, namespace);
