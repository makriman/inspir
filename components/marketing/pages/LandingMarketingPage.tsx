...
type LandingTranslator = (value: string) => string;

export function generateLandingMetadata(language: SupportedLanguage) {
  return localizeMarketingMetadataForLanguage(pageMetadata, "/", language);
}
...
export async function LandingPageContent({
  ...
  return (
    <main className="marketing-site">
      <JsonLdScripts items={jsonLd} language={language} />
      <MarketingHeaderWithChrome chrome={chrome} hero />
      <LandingHero t={t} chrome={chrome} />
      <LandingImpactBand proofStats={proofStats} t={t} />
      <TrustSignalsSection t={t} />
      <MissionStorySection t={t} />
      <LearningProcessSection t={t} hrefLanguage={hrefLanguage} />
      <ModeCardsSection t={t} />
      <PublicModesSection priorityTopics={priorityTopics} t={t} hrefLanguage={hrefLanguage} />
      <SubjectHubsSection subjectPages={subjectPages} t={t} hrefLanguage={hrefLanguage} />
      <LearningPathsSection t={t} hrefLanguage={hrefLanguage} />
      <ContentEngineSection posts={posts} t={t} hrefLanguage={hrefLanguage} />
      <BlogHighlightsSection featuredPosts={featuredPosts} t={t} hrefLanguage={hrefLanguage} />
      <SchoolsSection t={t} hrefLanguage={hrefLanguage} />
      <FaqSection t={t} />
      <GithubSection t={t} />
...
function LandingHero({ t, chrome }: { t: LandingTranslator; chrome: MarketingChrome }) {
  ...
  return (
    <section className="marketing-band marketing-impact-band">
      <div className="marketing-section-copy is-centered">
        <span>{t("Useful from the first click")}</span>
        <h2>{t("Built for curiosity, practice, and access.")}</h2>
        <p>
          {t(
...
function TrustSignalsSection({ t }: { t: LandingTranslator }) {
  return (
    <section className="marketing-band is-trust-signals" aria-labelledby="trust-signals-title">
      <div className="marketing-section-copy">
        <span>{t("Designed for confidence")}</span>
        <h2 id="trust-signals-title">{t("A public learning site people can understand quickly.")}</h2>
        <p>
          {t(
      ...
      <div className="marketing-story-copy">
        ...
        <div className="marketing-proof-grid">
          <div>
            ...
            {t("Built from real learner behavior, not a generic chat box.")}
          </div>
          <div>
            <CheckCircle2 size={20} />
            {t("Supports extracurricular learning, academic practice, and curiosity.")}
          </div>
          <div>
            <CheckCircle2 size={20} />
            {t("Available publicly while schools can run confidential custom versions.")}
          </div>
        </div>
      </div>
    </section>
...
function LearningProcessSection({ t, hrefLanguage }: { t: LandingTranslator; hrefLanguage: SupportedLanguage }) {
  return (
    <section className="marketing-band is-process" aria-labelledby="learning-process-title">
      <div className="marketing-section-copy">
        <span>{t("How it works")}</span>
        <h2 id="learning-process-title">{t("The site is a route, not a brochure.")}</h2>
        <p>
          {t(
...
function ModeCardsSection({ t }: { t: LandingTranslator }) {
  return (
    <section className="marketing-band">
      <div className="marketing-section-copy">
        <span>{t("What you can do")}</span>
        <h2>{t("Learn by talking, testing, debating, and exploring.")}</h2>
      </div>
      <div className="marketing-card-grid">
        {modes.map((mode) => {
...
function PublicModesSection({
  ...
  return (
    <section className="marketing-band is-discovery">
      <div className="marketing-section-copy">
        <span>{t("Public learning modes")}</span>
        <h2>{t("Open the exact kind of help you came for.")}</h2>
        <p>
          {t(
      ...
      <div className="marketing-inline-actions">
        <ArrowLink href="/chat/learn-anything" hrefLanguage={hrefLanguage}>
          {t("Open guest chat")}
        </ArrowLink>
        <ArrowLink href="/topics" hrefLanguage={hrefLanguage}>
          {t("Browse every mode")}
        </ArrowLink>
      </div>
    </section>
...
function SubjectHubsSection({
  subjectPages,
  t,
  hrefLanguage,
}: {
  subjectPages: SubjectPageSummary[];
  t: LandingTranslator;
  hrefLanguage: SupportedLanguage;
}) {
  return (
    <section className="marketing-band is-topic-finder">
      <div className="marketing-section-copy">
        <span>{t("AI tutors by subject")}</span>
        <h2>{t("Start from the subject, then open the right learning mode.")}</h2>
        <p>
          {t(
      ...
      <div className="marketing-inline-actions">
        <ArrowLink href="/subjects" hrefLanguage={hrefLanguage}>
          {t("Browse subjects")}
        </ArrowLink>
        <ArrowLink href="/ai-learning-map" hrefLanguage={hrefLanguage}>
          {t("Open the learning map")}
        </ArrowLink>
      </div>
    </section>
  );
...
function LearningPathsSection({ t, hrefLanguage }: { t: LandingTranslator; hrefLanguage: SupportedLanguage }) {
  return (
    <section className="marketing-band is-learning-paths">
      <div className="marketing-section-copy">
        <span>{t("Popular paths")}</span>
...
function ContentEngineSection({
  ...
  return (
    <section className="marketing-band is-guide-library" aria-labelledby="guide-library-title">
      <div className="marketing-section-copy">
        <span>{t("Guide library")}</span>
        <h2 id="guide-library-title">
          {t(`A ${posts.length}-guide library built for learning that turns into action.`)}
        </h2>
        <p>
          {t(
      ...
      <div className="marketing-card-grid">
        <article className="marketing-card">
          <LibraryBig size={24} />
          <h3>{t(`${posts.length} practical guides`)}</h3>
          <p>{t("Mode guides, prompt loops, study methods, and cornerstone articles for common learning moments.")}</p>
        </article>
        <article className="marketing-card">
          <Route size={24} />
          <h3>{t("Topic clusters")}</h3>
          <p>{t("Every article connects to categories, live modes, subject hubs, learning paths, and related guides.")}</p>
        </article>
        <article className="marketing-card">
          <Waypoints size={24} />
          <h3>{t("Easy next steps")}</h3>
          <p>{t("Each guide points to a mode, prompt, or review loop so reading can become practice.")}</p>
        </article>
        <article className="marketing-card">
          <BookOpenCheck size={24} />
          <h3>{t("Built to be used")}</h3>
          <p>{t("Each guide ends with practice plans, routes, and links into the exact learning mode that fits.")}</p>
        </article>
      </div>
      <div className="marketing-inline-actions">
        <ArrowLink href="/blog" hrefLanguage={hrefLanguage}>
          {t("Explore the guide library")}
        </ArrowLink>
        <ArrowLink href="/ai-learning-map" hrefLanguage={hrefLanguage}>
          {t("Open the learning map")}
        </ArrowLink>
...
function BlogHighlightsSection({
  ...
  return (
    <section className="marketing-band">
      <div className="marketing-section-copy">
        <span>{t("From the blog")}</span>
        <h2>{t("Study loops, AI tutoring guides, and practical prompts.")}</h2>
        <p>
          {t(
...
function SchoolsSection({ t, hrefLanguage }: { t: LandingTranslator; hrefLanguage: SupportedLanguage }) {
  return (
    <section className="marketing-split-band">
      <div>
        <span className="marketing-kicker dark">{t("For schools and CSR partners")}</span>
        <h2>{t("Custom AI learning spaces with data confidentiality.")}</h2>
        <p>
          {t(
        ...
        <div className="marketing-inline-actions">
          <ArrowLink href="/schools" hrefLanguage={hrefLanguage}>
            {t("Explore schools")}
          </ArrowLink>
      ...
      <div className="marketing-school-panel" aria-hidden="true">
        <School size={34} />
        <strong>{t("White-labelled school AI")}</strong>
        <span>{t("School-specific workflows")}</span>
        <span>{t("NCERT-aligned options")}</span>
        <span>{t("Confidential deployments")}</span>
      </div>
    </section>
  );
}

function FaqSection({ t }: { t: LandingTranslator }) {
  return (
    <section className="marketing-band is-home-faq">
      <div className="marketing-section-copy">
        <span>{t("Quick answers")}</span>
        <h2>{t("What learners usually ask before they start.")}</h2>
      </div>
...
function GithubSection({ t }: { t: LandingTranslator }) {
  return (
    <section className="marketing-band is-github">
      <div className="marketing-section-copy">
        <span>{t("Build with us")}</span>
        <h2>{t("inspir is being built in public.")}</h2>
        <p>
          {t(
      ...
      <div className="marketing-repo-grid">
        {repos.map((repo) => (
          <a key={repo.href} href={repo.href} target="_blank" rel="noreferrer" className="marketing-repo-card">
            ...
            <span>{t(repo.text)}</span>
          </a>
        ))}
      </div>
      <div className="marketing-contribute-row">
        <span>{t("Good first areas:")}</span>
        <p>{t("accessibility, prompts, tests, safer AI behavior, learner flows, and UI polish.")}</p>
      </div>
    </section>
  );
}
