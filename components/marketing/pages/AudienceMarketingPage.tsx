...
export function generateAudienceMetadata(language: SupportedLanguage) {
  return localizeMarketingMetadataForLanguage(pageMetadata, "/for", language);
}

export async function AudiencePageContent({
  language,
  pathname,
}: {
  ...
  return (
    <main className="marketing-site">
      <JsonLdScripts items={jsonLd} path="/for" language={language} />
      <MarketingHeaderWithChrome chrome={chrome} />
      <MarketingPageHero
        eyebrow="For learners"
        title="AI learning should fit the person using it."
        visual="audience"
        chrome={chrome}
      >
      ...
      <section className="marketing-band is-topic-finder">
        ...
        <div className="marketing-mode-finder-grid">
          {pages.map((page) => (
            <Link
              key={page.slug}
              href={localizeStaticSiteHref(audiencePath(page.slug), hrefLanguage)}
      ...
      <section className="marketing-band">
        ...
        <div className="learning-path-mode-list">
          {audienceHubSearchIntents.map((intent) => (
            <Link key={intent} href={localizeStaticSiteHref("/for", hrefLanguage)}>
              <SearchCheck size={15} />
              {intent}
            </Link>
          ))}
