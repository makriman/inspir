...
export function generateMediaMetadata(language: SupportedLanguage) {
  return localizeMarketingMetadataForLanguage(pageMetadata, "/media", language);
}

const highlightIcons = [UsersRound, Globe2, Award, Newspaper] as const;

export async function MediaPageContent({
  language,
  pathname,
}: {
  ...
  return (
    <main className="marketing-site">
      <JsonLdScripts items={jsonLd} path="/media" language={language} />
      <MarketingHeaderWithChrome chrome={chrome} />
      <MarketingPageHero eyebrow="Media" title="A citeable public record for inspir." visual="media" chrome={chrome}>
      ...
      <section className="marketing-split-band">
        <div>
          ...
          <div className="marketing-inline-actions">
            <ArrowLink href="/topics" hrefLanguage={hrefLanguage}>Browse public modes</ArrowLink>
            <ArrowLink href="/blog/category/ai-tutor" hrefLanguage={hrefLanguage}>AI tutor guides</ArrowLink>
          </div>
        </div>
        ...
        <div className="marketing-card-grid">
          {mediaCitationSnippets.map((snippet) => (
            <article key={snippet.title} className="marketing-card">
              ...
              <h3>{snippet.title}</h3>
              <p>{snippet.text}</p>
              <ArrowLink href={snippet.href} hrefLanguage={hrefLanguage}>Source page</ArrowLink>
            </article>
          ))}
        </div>
      </section>
