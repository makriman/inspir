...
The legacy API matrix is derived from the same generated availability manifest used for localized rendering. Every supported language has one complete main-app result, and the current published site matrix contains `marketing-shell`, `route:home`, and `route:mission` for every supported language: 70 + (70 × 3) = 280 complete release assets. The adapter performs one Static Assets lookup for a published pair. A known namespace outside that language's published matrix returns a private/no-store `404` without an asset lookup; an unknown namespace returns a private/no-store `400`; and a missing advertised asset is logged and fails closed with a private/no-store `503`. Incomplete responses are never materialized or served.
