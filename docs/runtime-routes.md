...
Authenticated `/api/chat` and `/api/chat/finalize`, and guest `/api/guest-chat`, reject an oversized `Content-Length` and cancel the unread body before `JSON.parse`. Signed-in chat does that before session HMAC or D1. Bodies that omit `Content-Length` are still streamed under the existing 20 KiB cap. Chat JSON deeper than six levels or wider than forty arrays and objects is rejected before `JSON.parse`. Guest history, 6,000-character turns, and finalize text keep their product limits.

With `MEMORY_REQUEST_VECTOR_QUERY` absent or any value other than `1`, `true`, `yes`, or `on`, authenticated chat does not embed the turn or query Vectorize. The bounded D1 memory batch still runs, with a slimmer projection: five saved memories at 400 characters, two profiles at 400 characters, five recent turns with shorter question, answer, and topic text, and at most eight summary sections projected to id, title, category, summary, and do-not-mention. Source-id arrays are not materialized. Provider SSE remains the upstream body. `limits.cpu_ms` stays unset. `wrangler.jsonc` is not changed here; an absent or non-truthy `MEMORY_REQUEST_VECTOR_QUERY` already keeps that embedding and Vectorize work off the request.
