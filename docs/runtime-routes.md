...
Request-path memory retrieval may still embed the current message and query Vectorize. That work is bounded for the 10 ms ceiling:
...
If a production tail sample for memory-on `/api/chat` reaches 8 ms, move the embedding and Vectorize query off that request. Leave `limits.cpu_ms` unset.
