...
9. Keep production on Workers Free with no paid-only `limits.cpu_ms` setting. Validate the exact uploaded version under live tail and require every sampled request to remain below the 10 ms Free-plan CPU ceiling. Authenticated chat does not embed or query Vectorize on the user request while `MEMORY_REQUEST_VECTOR_QUERY` is `0`. If that var is enabled later, embedding parses stay under 64 KiB and Vectorize chat queries stay at topK 8. Profile-photo hashing digests the upload view in place.
